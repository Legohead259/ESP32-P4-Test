...
const IPAddress kClientIP(192, 168, 34, 21);
const IPAddress kGateway(192, 168, 34, 1);
const IPAddress kNetmask(255, 255, 255, 0);
const IPAddress kAgentIP(192, 168, 34, 7);
...
void setup() {
  Serial.begin(115200);
  delay(2000);
  ESP_LOGI("INIT", "Starting micro-ROS node...");
...
void HandleConnectionState() {
  switch (connection_state) {
    ...
    // ESP_LOGI("HandleConnectionState", "[APP] Free memory: %d bytes", esp_get_free_heap_size());
    // ESP_LOGI("HandleConnectionState", "This Task watermark: %d bytes", uxTaskGetStackHighWaterMark(NULL));
      if (RMW_RET_OK == rmw_uros_ping_agent(20000, 3)) {
        ESP_LOGI("ROS", "Agent found, establishing connection...");
        connection_state = ConnectionState::kConnecting;
      }
      else {
        ESP_LOGW("ROS", "Failed to find agent. Retrying...");
      }
      break;
